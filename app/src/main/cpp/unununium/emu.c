...
static void step(void)
{
	...
	/*__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid",
					 "op0, opA, op1, opN, opimm: %x %x %x %x %x ",
					 op0, opA, op1, opN, opimm);
	*/
	// jumps
	if (opA == 7 && op1 < 2) {
		cycle_count += 2;
...
{
	int idle = 0;
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "Start Line loop");
	for (;;) {
//		if (timer_triggered != timer_handled)
//			break;

		if (trace){
			//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "trace");
			print_state();
		}

		if (trace_new && ever_ran_this[cs_pc()] == 0) {
			//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "S1");
			ever_ran_this[cs_pc()] = 1;
			print_state();
		}

		if (cs_pc() == board->idle_pc) {
			//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "S2");
			idle++;
			if (idle == 2) {
				cycle_count = 0;
				break;
		...
		if (cycle_count >= cycles_per_line) {
			//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "S3");
			cycle_count -= cycles_per_line;
			break;
		}

	}
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "Out of Loop");
}
...
//	last = now;
	...
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "Idle");
	do_idle();
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "Render");
	render();

//	now = get_realtime();
//	debug("-> %uus for this field (gfx)\n", now - last);
//	last = now;
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyDroid", "Controller");
	do_controller();
