...
{
	__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "Killing atlas");
	debug("Killing atlas\n");
...
static struct cache *cache_make(u32 key)
{
	//__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "make cache");
	struct cache *slot = &cache[cache_hash(key)];
	if (slot->key == -1U)
		//__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "return slot 1 %i", slot);
		return slot;

	for ( ; slot->next; slot = slot->next) {
		if (slot->key == key)
			//__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "return slot 2 %i", slot);
			return slot;
		if (slot->key == -1U)
			fatal("WHOOPS, CACHE WENT BAD (make)\n");
	}

	__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "make, new slot %04x\n", cache_free_head - cache);
    debug("make, new slot %04x\n", cache_free_head - cache);
	slot->next = cache_free_head;

	// the 3 lines of code that causes crashes for V Smile. Prob because when VSmile loads, cache is not initialised
	slot = cache_free_head;
	cache_free_head = cache_free_head->next;
	slot->next = 0;

	//__android_log_print(ANDROID_LOG_ERROR, "SonnyBoy", "return slot 3 %i", slot);
	return slot;
}
...
char cache_started = 0;
void render(void)
{
	if (atlas_y + atlas_h + (2*512*256 + 256*64*64) / ATLAS_W > ATLAS_H || cache_started == 0)
		render_kill_cache();
